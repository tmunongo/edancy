<!DOCTYPE html>
<html>

<head>
    <title>Sidebar</title>
</head>

<body>
    <aside class="">
        <!-- Add sidebar content here -->
        <ul class="nav flex-column nav-pills sidebar">
            <li class="nav-item sidebar-item" aria-current="page">
                <a href="http://www.edancy.co.zw/admin/index.php" class="nav-link">
                    <i class="fa fa-home" aria-hidden="true"></i> Home
                </a>
            </li>
            <li class="nav-item sidebar-item" aria-current="page">
                <a href="http://www.edancy.co.zw/admin/manage-home.php" class="nav-link">
                    <i class="fa fa-home" aria-hidden="true"></i> Update Home Page
                </a>
            </li>
            <li class="nav-item sidebar-item" aria-current="page">
                <a href="" class="nav-link">
                    <i class="fa fa-heart" aria-hidden="true"></i> Favorites
                </a>
            </li>
            <li class="nav-item sidebar-item" aria-current="page">
                <a href="http://www.edancy.co.zw/admin/posts/newpost.php" class="nav-link">
                    <i class="fas fa-notes-medical    "></i> New Post
                </a>
            </li>
            <li class="nav-item sidebar-item" aria-current="page">
                <a href="" class="nav-link">
                    <i class="fa fa-book" aria-hidden="true"></i> Categories
                </a>
            </li>
            <li class="nav-item sidebar-item" aria-current="page">
                <a href="http://www.edancy.co.zw/admin/utils/logout.php" class="nav-link">
                    <i class="fas fa-power-off    "></i> Logout
                </a>
            </li>
        </ul>
    </aside>
</body>

</html>